    ...
    <main className="min-h-screen bg-[#07070b] text-[#f5efe2]">
      <section className="mx-auto w-full max-w-7xl px-6 py-10 sm:px-10 lg:px-12">
        <nav className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-5 text-sm text-white/60">
          ...
          <div className="flex flex-wrap gap-3"><a href="/service-bridge/control">Control</a><a href="/api/service-bridge/openapi">OpenAPI</a><a href="/">Home</a></div>
        </nav>

        <header className="grid gap-8 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div>
            <p className="mb-4 inline-flex rounded-full border border-fuchsia-300/30 bg-fuchsia-300/10 px-4 py-2 text-sm text-fuchsia-100">Live manifest • health • receipt</p>
            <h1 className="text-5xl font-black tracking-tight sm:text-7xl">System Status</h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-white/70">One status console for runtime health, API capabilities, limits, verification commands, service inventory, and approval-boundary evidence.</p>
          </div>
          <div className={`rounded-[2rem] border p-6 ${healthy ? "border-emerald-300/30 bg-emerald-300/10" : "border-amber-300/30 bg-amber-300/10"}`}>
            ...
            <p className="mt-2 text-sm text-white/60">External action completed: NO</p>
          </div>
        </header>
        ...
        <section className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            ["Health checked", health?.checkedAt ?? "—"],
