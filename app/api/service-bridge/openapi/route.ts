...
export function GET() {
  return NextResponse.json({
    ...
    info: {
      ...
      version: "15.0.0",
      description: "Local-first orchestration, reversible recovery, and mission-scoped chained lifecycle journaling with explicit integrity limitations.",
    },
    servers: [{ url: "/" }],
    paths: {
    ...
    components: { schemas: {
      Mission: { type: "object", required: ["id", "title", "service", "target", "action", "owner", "state", "priority", "permission", "evidence", "fallback"] },
      MissionBatch: { type: "object", required: ["missions"] },
      MissionQueue: { type: "object", required: ["missions"] },
      ...
      EventChain: { type: "object", required: ["events"] },
    ...
    "x-recovery-stages": ["event-chain", "verify", "project", "reconcile", "resolve-authority", "plan-persistence", "explicit-local-write", "plan-rollback", "explicit-local-restore", "append-lifecycle-entry", "verify-lifecycle-journal"],
