...
type EventLink = { sequence: number; eventDigest: string; previousEventDigest: string | null };
...
export async function GET() {
  ...
  const digestChainConnected = events.every((event, index) => index === 0 ? event.previousEventDigest === null : event.previousEventDigest === events[index - 1]?.eventDigest);
  const headMatches = eventChain.headSequence === events.at(-1)?.sequence && eventChain.headDigest === events.at(-1)?.eventDigest;
  ...
  return Response.json({
    ...
    controllingIssues: { stateMachine: stateMachine.controllingIssue, canonicalApplication: application.controllingIssues },
    planningBaseline: stateMachine.baseline,
    capacity: {
      totalPlanningSlots: capacityLedger.canonicalCapacity.totalPlanningSlots,
      standardActiveCeiling: capacityLedger.canonicalCapacity.standardActiveCeiling,
    ...
    googleDriveContinuity: {
      ...
      documentTitles: googleDriveReceipt.documents.map((document) => document.title),
      ...
      receiptMeshMaintained: googleDriveReceipt.automation.maintainAppendOnlyReceiptMesh,
      conflictRegisterMaintained: googleDriveReceipt.automation.maintainTransitionConflictRegister,
      canonicalApplicationsMaintained: googleDriveReceipt.automation.maintainCanonicalEventApplications,
    },
    evidenceBoundary: {
    ...
    nextControlledAction: "Preserve sequence 1 and reconcile any later change through a new reviewed append-only event.",
