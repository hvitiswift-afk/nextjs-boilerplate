...
const requiredRevenue=["name: Verify Fardarter Drive v6.19 strategy authority map","name: Verify Fardarter Drive v6.21 current control head","npm run revenue:verify","FARDARTER-DRIVE-STRATEGY-RAIL-V6-19.json","FARDARTER-DRIVE-CONTROL-HEAD-V6-18.json","FARDARTER-DRIVE-CURRENT-CONTROL-HEAD-V6-21.json","fardarter-drive-strategy-rail-v6-19.schema.json","fardarter-drive-current-control-head-v6-21.schema.json","controller_version=6.19.0","controller_version=6.21.0","strategy_control=FARDARTER-DRIVE-STRATEGY-RAIL-V6-19","strategy_manifest_digest=2f17be6563d67a1a724731ea8c541f29daf1cda1e1cfaf490bcf8c50f77ac022","strategy_document_sha256=6396928240bfec30f3e366615fd43c4602e3a3c9966ffc3051d9ef86b8998990","issue_141_role=HISTORICAL_STRATEGY_AND_STAGE_GOVERNANCE_RAIL","standing_control=FARDARTER-DRIVE-CONTROL-HEAD-V6-18","control_head_manifest_digest=80764bd177469742831497a96290a8d74518035d632d4908f669006c3bf45f7c","permissions:\n  contents: read"];
requiredRevenue.forEach(x=>ok(t.revenue.includes(x),`revenue workflow missing ${x}`));
const lines=t.revenue.split("\n");
ok(lines.some(x=>x.trim()==="name: Verify Fardarter Drive v6.21 current control head"),"active v6.21 job missing");
ok(!lines.some(x=>x.trim()==="name: Verify Fardarter Drive v6.19 strategy authority map"),"v6.19 remains active instead of historical");
ok(!lines.some(x=>x.trim()==="name: Verify Fardarter Drive v6.18 standing control head"),"v6.18 remains active YAML job");
ok(t.revenue.includes("Historical v6.19 compatibility anchors")&&t.revenue.includes("historical_job_identity_v619=name: Verify Fardarter Drive v6.19 strategy authority map")&&t.revenue.includes("Historical v6.18 compatibility anchors")&&t.revenue.includes("historical_job_identity_v618=name: Verify Fardarter Drive v6.18 standing control head")&&t.revenue.includes("Historical v6.16 compatibility anchors"),"compatibility anchors missing");
...
ok(pkg.scripts["fardarter:control-head:check"]==="node scripts/check-fardarter-control-head-v6-18.mjs"&&pkg.scripts["fardarter:strategy-rail:check"]==="node scripts/check-fardarter-strategy-rail-v6-19.mjs"&&pkg.scripts["fardarter:current-control-head:check"]==="node scripts/check-fardarter-current-control-head-v6-21.mjs"&&pkg.scripts["revenue:verify"].includes("npm run fardarter:control-head:check")&&pkg.scripts["revenue:verify"].includes("npm run fardarter:current-control-head:check")&&pkg.scripts["revenue:verify"].endsWith("npm run fardarter:strategy-rail:check"),"package integration mismatch");
